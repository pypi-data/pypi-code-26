...
class TestFligner(object):
    ...
    def test_trimmed1(self):
        ...
        Xsq2, pval2 = stats.fligner(g1, g2, g3, center='trimmed',
                                    proportiontocut=0.0)
        # Value of statistic changes as inputs are perturbed by random machine epsilons, with standard deviation at 6.3e-3 warranting decimal == 2
        #
        #   In [67]: fVec = np.array([ stats.fligner(*[
        #    ...:        [ x + 0.5*u*np.finfo(np.double).eps for x, u in zip(g,np.random.randn(len(g)))] for g in (g1,g2,g3)
        ...
        #
        # Even though 'trimmed with proptioncut=0.0 does not change the dataset, it reorders it, and hence np.mean(x) may change.
        assert_almost_equal(Xsq1, Xsq2, decimal = 2)
        assert_almost_equal(pval1, pval2, decimal = 2)
